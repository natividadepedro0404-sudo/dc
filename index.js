...
app.post('/start', async (req, res) => {
  const { servers, batchSize, delay } = req.body;

  if (!servers || servers.length === 0) {
    return res.json({ error: 'Sem servidores' });
  }

  config.user_scan_batch_size = batchSize || 3;
  config.user_check_delay_ms = delay || 10000;

  console.log('🚀 Scan iniciado via painel');
...
content = content.replace(
  ...
  console.log('✅ Patch friend_source_flags aplicado com sucesso.');
} catch (error) {
...
async function badgeScrapper(guildId) {
  try {
    ...
    const batchSize = config.user_scan_batch_size || 3;
    ...
    for (let i = 0; i < memberIds.length; i += batchSize) {
      ...
      console.log(`[INFO] Aguardando ${checkDelay}ms para o próximo grupo...`.cyan);
...
function loadServerIds() {
  try {
    const data = readFileSync('./servers.txt', 'utf-8');
    ...
    console.error('❌ Erro ao ler servers.txt:', error.message);
    return [];
  }
}
...
client.once('ready', async () => {
  ...
  if (serverIds.length === 0) {
    console.log('❌ Nenhum ID encontrado em servers.txt');
    await client.destroy();
    process.exit(1);
  }

  console.log('[SERVER]'.bgYellow + ` ${serverIds.length} servidores carregados do servers.txt`.yellow);

  for (const guildId of serverIds) {
    console.log('\n[SERVER]'.bgYellow + ` Processando servidor: ${guildId}`.yellow);

    try {
      await fetchUsers(guildId);
      await badgeScrapper(guildId);
    } catch (error) {
      console.error(`❌ Erro ao processar servidor ${guildId}:`, error.message);
    }

    await delay(5000);
  }

  console.log('[COMPLETE]'.bgGreen + ' Todos os servidores foram processados.'.green);
